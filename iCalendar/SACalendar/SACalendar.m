...
- (UIViewController *) controllerAtIndex:(NSInteger) index {
    ...
    if (![controllers objectForKey:[NSString stringWithFormat:@"%li",(long)index]]) {
        ...
        UIView *daysView = [[UIView alloc]initWithFrame:CGRectMake(0, headerSize - 16, self.frame.size.width, 16)];
        UIFont * daysFont = [UIFont fontWithName:monthLabel.font.fontName size:8.0];
        UIColor * daysColor = [UIColor blackColor];
        UIColor * weekendColor = [UIColor lightGrayColor];
        CGFloat daysWidth = self.frame.size.width / 7 ;
       
        [monthLabel addSubview:daysView];
        
        UILabel *sunday = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, daysWidth,16)];
        sunday = [self configureDaysLabel:sunday withFont:daysFont withTextColor:weekendColor withText:@"SUN"];
        [daysView addSubview:sunday];
        
        UILabel *monday = [[UILabel alloc]initWithFrame:CGRectMake(sunday.frame.size.width, 0, daysWidth,16)];
        monday = [self configureDaysLabel:monday withFont:daysFont withTextColor:daysColor withText:@"MON"];
        [daysView addSubview:monday];

        UILabel *tuesday = [[UILabel alloc]initWithFrame:CGRectMake(monday.frame.origin.x + monday.frame.size.width, 0, daysWidth,16)];
        tuesday = [self configureDaysLabel:tuesday withFont:daysFont withTextColor:daysColor withText:@"TUE"];
        [daysView addSubview:tuesday];

        UILabel *wednesday = [[UILabel alloc]initWithFrame:CGRectMake(tuesday.frame.origin.x + tuesday.frame.size.width, 0, daysWidth,16)];
        wednesday = [self configureDaysLabel:wednesday withFont:daysFont withTextColor:daysColor withText:@"WED"];
        [daysView addSubview:wednesday];
        
        UILabel *thursday = [[UILabel alloc]initWithFrame:CGRectMake(wednesday.frame.origin.x + wednesday.frame.size.width, 0, daysWidth,16)];
        thursday = [self configureDaysLabel:thursday withFont:daysFont withTextColor:daysColor withText:@"THR"];
        [daysView addSubview:thursday];
        
        UILabel *friday = [[UILabel alloc]initWithFrame:CGRectMake(thursday.frame.origin.x + thursday.frame.size.width, 0, daysWidth,16)];
        friday = [self configureDaysLabel:friday withFont:daysFont withTextColor:daysColor withText:@"FRI"];
        [daysView addSubview:friday];
        
        UILabel *satrday = [[UILabel alloc]initWithFrame:CGRectMake(friday.frame.origin.x + friday.frame.size.width, 0, daysWidth,16)];
        satrday = [self configureDaysLabel:satrday withFont:daysFont withTextColor:weekendColor withText:@"SAT"];
        [daysView addSubview:satrday];
...
-(UILabel *)configureDaysLabel:(UILabel *)lable withFont:(UIFont *)font withTextColor:(UIColor *)color withText:(NSString *)text {
    lable.font = font;
    lable.text = text;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row < firstDay || indexPath.row >= firstDay + daysInMonth) {
        ...
        NSString * cellDate = [NSString stringWithFormat:@"%i",(int)indexPath.row - firstDay + 1];
        cell.dateLabel.text = cellDate;
        
        
        if (nil != _delegate && [_delegate respondsToSelector:@selector(SACalendar:subTitleForDate:month:year:)]) {
            
            cell.subLabel.text = [_delegate SACalendar:self subTitleForDate:cellDate.intValue month:current_month year:current_year];
        }
